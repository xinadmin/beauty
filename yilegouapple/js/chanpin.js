...
function over(item) {
	...
	for (var i = 0; i < items.length; i++) {
		...
		html += "<img src='" + items[i] +
			"' data-preview-src='' data-preview-group='1'  width='90%' height='70%'  /></a></li>";
	...
	vm.is_show = false;
	//	$("body #mui-slider-indicator .mui-indicator:first-child").addClass("mui-active");
}
//规格值选择
mui('body').on('tap', 'li', function() {
	$("body .sys_spec_text > li").removeClass("selected");
	...
	if ($(this).children("a").attr("title") == "" || $(this).children("a").attr("title") == null) {
		...
		if ($(this).children("a").attr("price") != null) {
			vm.promotionPrice = parseFloat($(this).children("a").attr("price")).toFixed(2);
			vm.discountPrice = (parseFloat(vm.promotionPrice) * 0.95).toFixed(2);
		}
	}
})
