...
function() {
	...
	function(a, b) {
		...
		function(a) {
		a.app_url = function() {
			return "http://app.zhonyou.cn/";
			//return "http://192.168.3.249/elgapp/";
		//	return "http://192.168.0.119/elgapp/";
//			return "http://192.168.43.122/elgapp/";
	
		}
	}(mui, window),
	function(a) {
		a.tel_bangding = function(openid) {
			if(openid) {
			var  btnArray = ['取消', '确定'];   //注意这里的顺序是先否再是
			mui.prompt("请绑定手机号码", "", '依乐购', btnArray, function(e) {
				if(e.index == 1) {
					if(!(/^1[345678]\d{9}$/.test(e.value.replace(/\s+/g, "")))) {
						mui.toast("请填写正确手机号码");
						return false;
					}else {
					var waiting = plus.nativeUI.showWaiting();
					mui.ajax({
						url:  "http://app.zhonyou.cn/SysUser/bindingUser",
						type: "POST",
						data: {
							phone: e.value,
							weixinId: openid
						...
						success: function(data) {
							...
							window.localStorage.setItem("user_tel", data.userTel);
							window.localStorage.setItem("is_member", data.isMember);
							window.localStorage.setItem("is_shop", data.isSystem);
							window.localStorage.setItem("idcard", data.idcard);
							window.localStorage.setItem("person_code", data.personCode);
							window.localStorage.setItem("yaoqing_code", data.yaoqingCode);
							window.localStorage.setItem("is_meirongshi", data.isMeirongshi);
							mui.toast("绑定成功,欢迎您使用");
						},
